...
// 找到1814687573@qq.com该用户后调用then函数，实现加密操作
User.findOne({ 'email': '1814687573@qq.com' }).then(async result => {
	if (result == null) {
		// 生成盐
		const salt = await bcrypt.genSalt(10);
		// 使用盐对密码进行加密
		const password = await bcrypt.hash('123456', salt);
		// 往集合里面创建了一个文档，这是默认的管理员，每次启动时都会创建，所以无法删除
		const user = await User.create({
			nickName: 'oldwei',
			email: '1814687573@qq.com',
			password: password,
			role: 'admin',
			avatar: null,
			createTime: new Date,
